...
function reloadListView(todoList: Todo[]): void {
  ...
  for (const todo of todoList) {
    ...
    ul.appendChild(tmpl);
    // イベントリスナーを追加
    const ckBtn = tmpl.getElementsByClassName(
      "todo-toggle__checkmark"
    )[0] as HTMLElement;
    const ck = tmpl.getElementsByClassName(
      "todo-toggle"
  ...
  document.getElementsByClassName(
    "done-items-num__value"
  )[0].innerHTML = String(checkedCounter);
}
